# create new hash with key/value pairs based on condition
...
CSV.foreach("sample1.csv", headers: true, header_converters: :symbol) do |row|  
  ...
  tap_hash = Hash.new.tap do |hash|
    ...
    hash[:emails] = Hash.new.tap do |email|
      email[:email] = email_list << row[:email] if row[:email]
    end
    hash[:phones] = Hash.new.tap do |phone|
      phone[:type] = phone_list << "home" if row[:homephone]
      phone[:phone] = phone_list << row[:homephone] if row[:homephone]
      phone[:type] = phone_list << "work" if row[:workphone]
      phone[:phone] = phone_list << row[:workphone] if row[:workphone]
    end
#     {
#       email: "" 
#     }
...
#     birthdayAt: @birthday_at
  ...
  # puts JSON.pretty_generate(@convert_to_json_hash.compact)
...
values = Hash.new.tap do |my_hash|
  my_hash[:x] = 1 
  my_hash[:y] = 2 
end
